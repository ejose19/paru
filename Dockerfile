...
# Required by buildx as these files are mounted as read-only (see https://github.com/moby/buildkit/issues/1267)
RUN sed -i "s~#\(NoExtract   =\)~\1 etc/hosts etc/resolv.conf~" /etc/pacman.conf
