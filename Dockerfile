FROM lopsided/archlinux:devel

WORKDIR /app

COPY ../ .

RUN if [[ $(uname -m) = "x86_64" ]]; then \
      glibc_filename=glibc-2.33-2-x86_64.pkg.tar.zst && \
      mkdir ci-fix && \
      cd ci-fix && \
      curl -O https://archive.archlinux.org/packages/g/glibc/$glibc_filename && \
      curl -O https://archive.archlinux.org/packages/g/glibc/${glibc_filename}.sig && \
      pacman-key --verify ${glibc_filename}.sig && \
      bsdtar -C / -xvf "$glibc_filename" && \
      cd .. && \
      rm -rf -- ci-fix && \
      sed -i "s/#\(IgnorePkg   =\)/\1 glibc/" /etc/pacman.conf; \
    fi

RUN echo "keyserver hkp://keyserver.ubuntu.com" >> /etc/pacman.d/gnupg/gpg.conf
RUN sed -i 's/CheckSpace/#CheckSpace/g' /etc/pacman.conf
RUN echo "[options]" >> /etc/pacman.conf
RUN echo "NoExtract = etc/hosts etc/resolv.conf" >> /etc/pacman.conf
RUN pacman -Sy --noconfirm archlinux-keyring
RUN pacman -Su --noconfirm rust
RUN ls -la
RUN ./dist
